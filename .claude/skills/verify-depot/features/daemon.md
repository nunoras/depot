...
- `no-credential` refuses to start when neither `gh` nor `<home>/github-token` has a token.
- `lock-record` writes `depotd.lock` with the pid, the projects in scope and a heartbeat.
- `single-instance` refuses a second `depotd` on the same home while the first keeps running.
- `heartbeat` advances the lock heartbeat each tick.
...
- Read `<home>/depotd.lock` or `depot status` to see whether a daemon drives a project.
...
- **Start.** The driver writes a placeholder `github-token` (mode 600) and starts `depotd` in the background, then waits until `depotd.lock` holds `"pid":<that pid>` and `"projects":["<repo>"]`.
- **Second instance.** `depotd` again.
  Exit 1, stderr `another depot daemon already holds <home>/depotd.lock`, and the first pid is still alive.
- **Status.** `depot status` still shows `## Held - awaiting approval (1)`.
- **Stop and restart.** The driver sends SIGTERM to the first pid, checks it is gone and that `depotd-1.log` has `daemon_restarted:<pid>:` and `polled:` facts, then starts a second daemon and stops it the same way.
...
- The heartbeat check fails at `e09a536`.
  `InstanceLock::acquire` opens `depotd.lock` without read access, so `refresh_heartbeat` cannot read the record and returns quietly.
  Everything before it in the recipe passes.
  Until that is fixed, `depot status` calls a live daemon stale after three poll intervals.
