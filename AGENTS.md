...
- `Store::apply_fact` (`crates/depotd/src/store/apply.rs`) is the one write path: reduce the fact, write the changed tasks, the coordinator row and the journal entry in one transaction, then the checklist. The CLI uses it today and the daemon will, and the actions it returns are the daemon's to execute.
