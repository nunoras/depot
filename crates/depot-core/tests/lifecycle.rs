...
fn rule_05_the_concurrency_cap_queues_work_and_frees_it() {
    ...
    run(vec![
        ...
        .checking(|state| subject(state, "t1").state == TaskState::Landed),
    ]);
}
...
fn rule_07_rate_limits_retry_with_bounded_backoff_then_pause() {
    ...
    run(vec![
        ...
        case(
            ...
            state(vec![with_attempt(running("t1"), spent(BUILD))]),
            vec![fact(1_000, limited(BUILD))],
        )
        .when(
            "t1",
        ...
        case(
            ...
            state(vec![with_attempts(
                with_attempt(task("t1", TaskState::Running), spent(BUILD)),
                3,
            )]),
            vec![fact(1_000, limited(BUILD))],
        )
